...
// LISTEN FIRST: sobe o servidor imediatamente para o proxy não retornar 503
let appReady = false;
app.get('/health', (req, res) => res.status(200).send('ok'));
app.use((req, res, next) => {
  if (!appReady) return res.status(503).set('Retry-After', '5').send('Carregando...');
  next();
});

const server = app.listen(PORT, HOST, () => {
  console.log('MaisFreelas http://%s:%s', HOST, PORT);
  loadFullApp().then(() => { appReady = true; console.log('[App] Rotas prontas.'); }).catch((e) => {
    console.error('[App] Falha ao carregar:', e.message);
  });
});
server.on('error', (e) => {
...
async function loadFullApp() {
  ...
  // Remove o middleware de "carregando" para inserir os de verdade no lugar certo
  // (o middleware atual fica; appReady true faz next() passar)
  app.get('/health-db', wrapAsync(async (req, res) => {
    try {
  ...
  db.testConnection()
    ...
    .then((switched) => switched ? ensureDatabase() : null)
    .then(() => {
